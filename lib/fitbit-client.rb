...
module FitbitClient
  ...
  VALID_SCOPES = %w(activity heartrate location nutrition profile settings sleep social weight)
  ...
  def self.array_to_scopes(scopes_array)
    raise 'You must specify a scope' if scopes_array.nil? || scopes_array.length < 1
